...
public:
    ...
    lidarImageProjection() {
        ...
        // detection_cone_sub = new message_filters::Subscriber<yolov7_ros::DetectionInfo>(nh, "/cone_box_multi", 1);
        // detection_del_sub = new message_filters::Subscriber<yolov7_ros::DetectionInfo>(nh, "/del_box_multi", 1);
        ...
        sf_info_pub = nh.advertise<cam_lidar_calib::sf_Info>("sf_info", 1000);
    ...
    void callback(const sensor_msgs::PointCloud2ConstPtr &cloud_msg,
        ...
        for (int i = 0; i < num_boxes; i += 4) {
            ...
            if(project_only_plane) {
                ...
                for(size_t i = 0; i < in_cloud->points.size(); i++) {
                    ...
                    //ROS_INFO("1_Lidar : x = %lf , y = %lf , z= %lf",pointCloud_L[0],pointCloud_L[1],pointCloud_L[2]);
                    //ROS_INFO("2_Lidar : x = %lf , y = %lf , z= %lf", X,Y,Z);
                    ...
                    if (X >= x_min && X <= x_max && Y>= y_min && Y<= y_max){
                        ...
                        if( l_y_max[k] < Y && X >= (box_center-10) && X <= (box_center+10)){
                            ...
                            */
                            sf_x[k] = pointCloud_L[0];  //라이다에서는 y(가로)
                            sf_y[k] = pointCloud_L[1];  //라이다에서는 z(세로)
                            sf_z[k] = pointCloud_L[2];  //라이다에서는 x(거리)
        ...
        //publish
        cam_lidar_calib::sf_Info sf_msg;
